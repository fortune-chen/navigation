...
ImuProcessor::ImuProcessor(const double acc_noise, const double gyro_noise,
    ...
    : m_accNoise(acc_noise), m_gyroNoise(gyro_noise), m_accBiasNoise(acc_bias_noise), m_gyroBiasNoise(gyro_bias_noise), m_gravity(gravity) {
}
...
void ImuProcessor::Predict(const ImuDataPtr last_imu, const ImuDataPtr cur_imu, State* state) {
    ...
    // just trust gps position is accurate
    // state->G_p_I = last_state.G_p_I + last_state.G_v_I * delta_t +
    //                0.5 * (last_state.G_R_I * acc_unbias + m_gravity) * delta_t2;
    state->G_v_I = last_state.G_v_I + (last_state.G_R_I * acc_unbias + m_gravity) * delta_t;
    ...
    Qi.block<3, 3>(0, 0) = delta_t2 * m_accNoise * Eigen::Matrix3d::Identity();
    Qi.block<3, 3>(3, 3) = delta_t2 * m_gyroNoise * Eigen::Matrix3d::Identity();
    Qi.block<3, 3>(6, 6) = delta_t * m_accBiasNoise * Eigen::Matrix3d::Identity();
    Qi.block<3, 3>(9, 9) = delta_t * m_gyroBiasNoise * Eigen::Matrix3d::Identity();
