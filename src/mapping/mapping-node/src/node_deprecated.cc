...
Node::Node(
    ...
    tf2_ros::Buffer* const tf_buffer, const bool collect_metrics, std::string& local_ip, std::string& sensor_bridge_ip)
    : node_options_(node_options), trajectory_options_(trajectory_options), is_mapping_(false),
  ...
  // wall_timers_.push_back(node_handle_.createWallTimer(
  //     ::ros::WallDuration(kGridmapPublishPeriodSec),
  //     &Node::PublishGridmap, this));
  sensorBridgeClient_ = absl::make_unique<SensorBridgeClient>(local_ip, sensor_bridge_ip);
  sensorBridgeClient_->registerControlCommandCallback([this](int flag){
...
void Node::PublishGridmap(std::map<SubmapId, SubmapSlice>&submapSlices) {
  ...
  sensorBridgeClient_->postGridMap(*msg_ptr);
}
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      ...
      if (node_options_.publish_tracked_pose) {
        ...
        sensorBridgeClient_->postPose(pose_msg);
      }
    }
  }
}
...
bool Node::HandleStartTrajectory(
  ...
  // LOG(WARNING) << "request.configuration_directory: " << request.configuration_directory;
  // LOG(WARNING) << "request.configuration_basename: " << request.configuration_basename; 
...
void Node::startMapping() {
  is_mapping_ = true;

  int trajectory_id = StartTrajectoryWithDefaultTopics(trajectory_options_);
  sensorBridgeClient_->registerOdomCallback([this,trajectory_id](const nav_msgs::OdometryConstPtr& msg){
    this->HandleOdometryMessage(trajectory_id, "odom", msg);
  });
#if USED_MULTI_ECHO_SCAN
  sensorBridgeClient_->registerLaserScanCallback([this,trajectory_id](const sensor_msgs::MultiEchoLaserScanConstPtr& msg){
    this->HandleMultiEchoLaserScanMessage(trajectory_id, "echoes", msg); });
#else
  sensorBridgeClient_->registerLaserScanCallback([this,trajectory_id](const sensor_msgs::LaserScanConstPtr& msg){
    this->HandleLaserScanMessage(trajectory_id, "scan", msg); });
#endif
  sensorBridgeClient_->registerImuCallback([this,trajectory_id](const sensor_msgs::ImuConstPtr& msg){
    this->HandleImuMessage(trajectory_id, "imu", msg);
  });
  sensorBridgeClient_->start(true);

  publishMapTimer_.StartTimer(10000, [this, trajectory_id](){ // default value 1000ms
    this->PublishSubmapList(trajectory_id);
  });
  updatePoseByTrajectoryTimer_.StartTimer(100, [this](){ // default value 5ms
    this->PublishLocalTrajectoryData();
  });
}

void Node::stopMapping() {
  sensorBridgeClient_->start(false);
  //RunFinalOptimization();
  publishMapTimer_.Expire();
  updatePoseByTrajectoryTimer_.Expire();

  FinishAllTrajectories();
  is_mapping_ = false;
}

void Node::writeStatus() {
  cartographer_ros_msgs::WriteState::Request request;
  cartographer_ros_msgs::WriteState::Response response;
  request.filename = "TrajectoryData" + std::to_string(trajectory_id_) + ".pbstream";
  request.include_unfinished_submaps = true;

  auto ret = HandleWriteState(request, response);
  std::cout << "ret : " << ret << " response status:" << response.status.message << std::endl;
}

void Node::startLocalization() {
   is_mapping_ = true;
  ...
  *trajectory_options.trajectory_builder_options.mutable_pure_localization_trimmer() = trimmer;
  ...
  int trajectory_id = StartTrajectoryWithDefaultTopics(trajectory_options);
#if USED_MULTI_ECHO_SCAN
  sensorBridgeClient_->registerLaserScanCallback([this,trajectory_id](const sensor_msgs::MultiEchoLaserScanConstPtr& msg){
    this->HandleMultiEchoLaserScanMessage(trajectory_id, "echoes", msg); });
#else
