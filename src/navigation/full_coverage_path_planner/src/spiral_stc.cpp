                         ...
                         std::vector<geometry_msgs::PoseStamped>& plan)
{
  if (!initialized_)
  {
    ...
    ROS_WARN("SpiralSTC can not make plan without a map");
    return false;
  } else
  {
  ...
  if (cpp_grid_.data.empty())
  {
    ROS_ERROR("Could not retrieve grid");
    return false;
  }

  if (!parseGrid(cpp_grid_, grid, robot_radius_ * 2, tool_radius_ * 2, start, startPoint))
  {
