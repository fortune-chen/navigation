...
bool LatchedStopRotateController::stopWithAccLimits(const geometry_msgs::PoseStamped& global_pose,
  ...
  if(valid_cmd){
    ...
    printf("latched_stop_rotate, Slowing down... using vx, vy, vth: %.2f, %.2f, %.2f\n", vx, vy, vth);
    cmd_vel.linear.x = vx;
    cmd_vel.linear.y = vy;
...
bool LatchedStopRotateController::rotateToGoal(
  ...
  if (valid_cmd) {
    ...
    ROS_DEBUG("dwa_local_planner, Moving to desired goal orientation, th cmd: %.2f, valid_cmd: %d\n", v_theta_samp, valid_cmd);
